...
def update_collection(api_client):
    def do_update_collection(collection, title):
        return api_client.patch(f'/store/collections/{collection.id}/', title)
...
class TestUpdateCollection:
    def test_if_user_is_anonymous_returns_401(self, update_collection):
        ...
        response = update_collection(collection, {'title': 'b'})
    ...
    def test_if_data_is_invalid_returns_400(self, authenticate, update_collection):
        ...
        response = update_collection(collection, {'title': ''})

        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.data['title'] is not None

    def test_if_data_is_valid_returns_200(self, authenticate, update_collection):
        authenticate(is_staff=True)
        collection = baker.make(Collection)
